...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/12 11:00:26 by wprintes          #+#    #+#             */
/*   Updated: 2023/11/12 16:58:37 by wprintes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <typename T>
void binaryInsert(T &vec, const typename T::value_type &value) {
...
void PmergeMe::_createChainPend(T &a, T &chain, T &pend)
{
    ...
    while (i < a.size()) {
        binaryInsert(chain, a[i - 1]);
        binaryInsert(pend, a[i]);
        i += 2;
    }

    if (a.size() % 2 == 1)
        binaryInsert(pend, a[i - 1]);
}
